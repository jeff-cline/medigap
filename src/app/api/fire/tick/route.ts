...
async function run(req: NextRequest) {
  ...
  // Notification Service rides the same cron: send any due calendar notifications to enrolled members.
  const notifications = await sendDueNotifications().catch(() => ({ events: 0, sent: 0, failed: 0 }));
  return NextResponse.json({ ...result, voicedrip: { ...vdScheduled, ...vdTick, ...vdCost }, followups, notifications });
}
