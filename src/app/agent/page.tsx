...
import PartnerStatement from "@/components/portal/PartnerStatement";
import BrandingEditor from "@/components/portal/BrandingEditor";
import { currentStatement } from "@/lib/accounting";
...
export default async function AgentPortal() {
  ...
  const F = (k: Parameters<typeof hasFeature>[1]) => hasFeature(agent?.features, k);
  const mySite = await db.site.findFirst({ where: { ownerId: session.uid }, orderBy: { createdAt: "desc" } });
  const statement = F("statement") ? await currentStatement(session.uid) : null;
  ...
  return (
    <>
      ...
      {F("branding") && mySite && (
      <Section title="Your branding" desc="Upload your logo, set your colors, hero headline, and your own footer links — applied to your white-label site.">
        <BrandingEditor site={{ id: mySite.id, name: mySite.name, logoUrl: mySite.logoUrl, brandColor: mySite.brandColor, heroHeadline: mySite.heroHeadline, footerLinks: mySite.footerLinks }} />
      </Section>
      )}
      ...
      {statement && (
      <Section title="Payout Statement" desc="Your rev-share earned last month, less your seat fees and call charges — paid on the 21st.">
        <PartnerStatement statement={statement} />
      </Section>
      )}
