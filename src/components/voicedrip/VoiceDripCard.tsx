...
export default function VoiceDripCard() {
  ...
  // The exact voicemail we drop to everyone (media/voicedrip-msg1.wav).
  const playMsg = () => { const a = audioRef.current; if (a) { a.currentTime = 0; a.play().catch(() => {}); } };
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] overflow-hidden">
      {/* the exact voicemail dropped to every recipient */}
      <audio ref={audioRef} src="/api/voicedrip/audio" preload="none" />
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] p-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={playMsg} title="Hear the exact voicemail being left" className="rounded-lg border border-[var(--border)] px-3 py-2 text-sm font-semibold hover:bg-[var(--panel2)]">▶ Hear the voicemail</button>
          <button
            disabled={saving}
      ...
      <div className="border-t border-[var(--border)] p-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-[10px] uppercase text-[var(--muted)]"><th className="text-left py-1">When</th><th className="text-left py-1">To</th><th className="text-left py-1">Result</th><th className="text-left py-1">Sec</th><th className="text-left py-1">Cost</th><th className="text-left py-1">Hear</th></tr></thead>
            <tbody>
              {recent.map((d) => {
                ...
                return (
                  <tr key={d.id} className="border-t border-[var(--border)]">
                    ...
                    <td className="py-1.5"><button onClick={playMsg} title={vm ? "Play the voicemail left" : "Play the voicemail message we drop"} className={`text-sm ${vm ? "text-[#22c55e]" : "text-[var(--brand)]"} hover:opacity-70`}>▶</button></td>
                  </tr>
                );
              })}
              {recent.length === 0 && <tr><td colSpan={6} className="py-4 text-center text-[var(--muted)] text-xs">No drops yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
