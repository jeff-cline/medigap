...
// Sends transactional email through a Zapmail-provisioned mailbox over SMTP.
// Config lives on the "zapmail" integration: smtpHost, smtpPort, smtpUser, smtpPass, fromEmail.
async function getTransport() {
  ...
  return {
    ...
    transport: nodemailer.createTransport({ host: c.smtpHost, port, secure: port === 465, auth: { user: c.smtpUser, pass: c.smtpPass } }),
  };
}

export async function sendEmail(to: string, subject: string, html: string): Promise<{ ok: boolean; error?: string }> {
  const t = await getTransport();
  if (!t) return { ok: false, error: "Zapmail SMTP not configured" };
  try {
...
const BRAND = "#16d6a5";
// Branded HTML for the "new account created" alert to the God inbox.
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e6e8ee">
    ...
    <div style="padding:24px">
      ...
      <table style="width:100%;border-collapse:collapse;margin-top:16px">
        ${row("Email", u.email)}${row("Phone", u.phone)}${row("Role", u.role)}${row("Came in from", u.source)}
      </table>
...
// Notify the God inbox of a new account.
export async function notifyNewAccount(u: { name: string; email: string; role: string; phone: string; source: string; id: string }) {
  return sendEmail("jeff.cline@me.com", `New ${u.role} account: ${u.name || u.email}`, newAccountEmail(u));
}
