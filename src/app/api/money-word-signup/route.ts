...
export async function POST(req: NextRequest) {
  ...
  if (!user) {
    user = await db.user.create({ data: { email, name: String(b.name || "").trim() || email.split("@")[0], role: "moneywords", passwordHash: await bcrypt.hash(tempPassword, 10), mustChangePassword: true, status: "active" } });
  }
  // Their first money-word bid, pre-populated for their area.
  await db.agentBid.create({ data: { agentId: user.id, scope, scopeValue: scope === "national" ? "" : scopeValue, keyword: word, amountCents: minCallBidCents, active: false } });
