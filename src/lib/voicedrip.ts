...
export async function reconcileVoiceDropCosts(now = Date.now()): Promise<{ reconciled: number }> {
  ...
  // Terminal Twilio statuses never change again → finalize them even if their price is null (no-answer/
  // busy/failed are simply not charged). This drains the queue so priceable drops actually get reached.
  ...
  for (const v of pending) {
    ...
    const hasPrice = j.price !== null && j.price !== undefined && j.price !== "";
    if (hasPrice) {
      data.priceCents = Math.round(Math.abs(parseFloat(j.price as string)) * 100);
      data.priceFinal = true;
      n++;
    } else if (TERMINAL.has(String(j.status || ""))) {
      data.priceCents = 0;
      data.priceFinal = true; // terminal + no charge → final at $0 so it stops re-queuing
    }
