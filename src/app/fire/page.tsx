...
export default async function FirePage({ searchParams }: { searchParams: Promise<{ tab?: string; ok?: string; date?: string; time?: string; win?: string; anchor?: string; preset?: string; cogs?: string }> }) {
  ...
  const tab = sp.tab === "calls" ? "calls" : sp.tab === "tv" ? "tv" : sp.tab === "tvage" ? "tvage" : sp.tab === "channels" ? "channels" : "emails";
  const isTvTab = tab === "tv" || tab === "tvage";

  const [lists, campaigns, contactTotal, sentTotal, openedTotal, conversions, dropsSent, dropSpend, voicemailsLeft] = await Promise.all([
    ...
    db.voiceDrop.count({ where: { direction: "outbound", answeredBy: { startsWith: "machine" } } }),
  ]);
  ...
  const emailCostCents = sentTotal * COST_PER_EMAIL;
  const costCents = emailCostCents + dropCostCents;
  const activities = sentTotal + dropsSent; // total outbound touches (email + voicemail drops)
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <Section title="Conversion tracker" desc="Every outbound email, and the ones who called back (green). Click a call to watch the voice‑AI conversation.">
          <div className="mb-3 flex gap-2">
            ...
            <Link href="/fire?tab=channels" className={`rounded-lg px-3 py-1.5 text-sm border ${tab === "channels" ? "bg-[#f59e0b]/10 text-[#f59e0b] border-[#f59e0b]/40" : "text-[var(--muted)] border-[var(--border)]"}`}>📊 VoiceDrip vs Email</Link>
          </div>
          <Card className="!p-0 overflow-hidden">
            {tab === "emails" ? (
              ...
              <div className="p-4">
                <div className="mb-4 text-xs text-[var(--muted)]">Both outbound channels run off the same list. <b>Callbacks can&rsquo;t be split by channel</b> — each person gets the email <i>and</i> the voicemail — so conversions, revenue and ROAS below are the <b>combined</b> result of the multi‑touch sequence.</div>

                <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-[var(--muted)]">Combined — email + VoiceDrip</div>
                <div className="mb-6 grid gap-3 sm:grid-cols-3 lg:grid-cols-5">
                  <Stat label="Total activities" value={num(activities)} sub={`${num(sentTotal)} emails + ${num(dropsSent)} drops`} tone="default" />
                  <Stat label="Conversions" value={num(conversions)} sub="callbacks → green" tone="up" />
                  <Stat label="Revenue" value={usd2(revenueCents)} sub={`${num(conversions)} × $75`} tone="up" />
                  <Stat label="Cost (COGS)" value={usd2(costCents)} sub="emails + drops" tone="down" />
                  <div className="rounded-2xl border border-[#22c55e]/40 bg-[#22c55e]/5 p-4">
                    <div className="text-xs uppercase text-[var(--muted)]">ROAS</div>
                    <div className="mt-1 text-3xl font-bold text-[#22c55e]">{roas.toFixed(1)}×</div>
                    <div className="text-xs text-[var(--muted)]">revenue ÷ cost</div>
