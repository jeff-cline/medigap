...
export async function GET(req: NextRequest) {
  ...
  for (const c of conns) {
    ...
    captured += r.captured;
    results.push({ account: c.accountName || c.userId, captured: r.captured, error: r.error });
  }
  return NextResponse.json({ ok: true, accounts: conns.length, captured, results, at: new Date().toISOString() });
}
