  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      nodesRef.current.forEach((nd) => {
        ...
        ctx.fillText(isCenter ? "R0CKETSHIP" : nd.node.label, nd.x, nd.y + (isCenter ? 0 : 0));
      });
        ...
        <div className="ag-panel p-6 min-h-[200px]">
          {active ? (
            ...
            <>
              ...
              <p className="mt-3 text-[var(--ag-muted)] leading-relaxed">Hover any node to see its revenue pathway. Every connection routes back through the R0cketShip trust layer — and each new node makes every other node more valuable.</p>
            </>
          )}
        </div>
