...
export async function POST(req: NextRequest) {
  ...
  // Classic form POST, so the silent success is a redirect to the thanks page
  // rather than a JSON body — a bot must not be able to tell it was dropped.
  const fields = Object.fromEntries([...fd.entries()].map(([k, v]) => [k, String(v)]));
  const gate = await guardForm(req, "rocketship_order", fields, {
    honeypot: g("website"),
    texts: [g("firstName"), g("lastName"), g("company"), g("notes"), g("goals")],
    email: g("email"), phone: g("phone"),
  });
  if (gate.blocked) return seeOther("/rocketship/thanks");
