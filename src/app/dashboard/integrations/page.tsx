...
const ITEMS: (IntegrationMeta & { phase: string })[] = [
  ...
  },
  {
    ...
    steps: ["Meta for Developers → your app → add the Facebook Login + Instagram Graph API products.", "Activate permissions (App Review → Permissions): read_insights, instagram_basic, instagram_manage_insights — Advanced Access for others' pages, Standard for your own/testers.", "FASTEST for real data: Business Settings → System Users → create one, assign your Pages + IG accounts, Generate Token (scopes: pages_show_list, pages_read_engagement, read_insights, instagram_basic, instagram_manage_insights, business_management), and paste it as the System User Token below.", "Then hit Pull metrics now on Social Metrics — no per-user OAuth needed for pages you own.", "Once permissions are activated, you can also use the Connect button; add the extra scopes in the Scope field."],
    fields: [{ name: "appId", label: "Meta App ID" }, { name: "appSecret", label: "Meta App Secret", type: "password" }, { name: "businessId", label: "Business Portfolio ID (pulls the whole portfolio)", placeholder: "1531355990484848" }, { name: "accessToken", label: "System User / Page Access Token (real data, no OAuth)", type: "password" }, { name: "scope", label: "OAuth scope override (add insights/IG once activated)", placeholder: "public_profile,pages_show_list,pages_read_engagement,business_management" }],
  },
  {
